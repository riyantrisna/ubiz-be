...
func (repository *LangRepositoryImpl) Save(ctx context.Context, tx *sql.Tx, lang domain.Lang) domain.Lang {
	...
	SQL := "INSERT INTO lang(lang_code, lang_name, created_by, created_at) VALUES (?, ?, ?, ?)"
	result, err := tx.ExecContext(ctx, SQL, lang.LangCode, lang.LangName, lang.CreatedBy, lang.CreatedAt)
...
func (repository *LangRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, lang domain.Lang) domain.Lang {
	SQL := "UPDATE lang SET lang_code = ?, lang_name = ?, updated_by = ?, updated_at = ? WHERE lang_id = ?"
	_, err := tx.ExecContext(ctx, SQL, lang.LangCode, lang.LangName, lang.UpdatedBy, lang.UpdatedAt, lang.LangId)
	helper.PanicIfError(err)
...
func (repository *LangRepositoryImpl) FindById(ctx context.Context, tx *sql.Tx, langId int) (domain.Lang, error) {
	...
	lang := domain.Lang{}
	if rows.Next() {
		err := rows.Scan(&lang.LangId, &lang.LangCode, &lang.LangName, &lang.CreatedBy, &lang.CreatedAt, &lang.UpdatedBy, &lang.UpdatedAt)
		helper.PanicIfError(err)
	}
