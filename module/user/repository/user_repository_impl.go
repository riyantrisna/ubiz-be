...
func (repository *UserRepositoryImpl) Save(ctx context.Context, tx *sql.Tx, user domain.User) domain.User {
	...
	SQL := "INSERT INTO user(user_name, user_email, user_password, user_lang_code, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?)"
	result, err := tx.ExecContext(ctx, SQL, user.UserName, user.UserEmail, user.UserPassword, user.UserLangCode, user.CreatedBy, user.CreatedAt)
...
func (repository *UserRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, user domain.User) domain.User {
	SQL := "UPDATE user SET user_name = ?, user_email = ?, user_lang_code = ?, updated_by = ?, updated_at = ? WHERE user_id = ?"
	_, err := tx.ExecContext(ctx, SQL, user.UserName, user.UserEmail, user.UserLangCode, user.UpdatedBy, user.UpdatedAt, user.UserId)
	helper.PanicIfError(err)
...
func (repository *UserRepositoryImpl) FindByTokenRefresh(ctx context.Context, tx *sql.Tx, userTokenRefresh string) (domain.User, error) {
	SQL := "SELECT user_id, user_name, user_password, user_lang_code FROM user WHERE user_token_refresh = ?"
...
func (repository *UserRepositoryImpl) UpdateToken(ctx context.Context, tx *sql.Tx, user domain.User) domain.User {
	SQL := "UPDATE user SET user_token = ?, user_token_refresh = ?, user_last_login = ? WHERE user_id = ?"
	_, err := tx.ExecContext(ctx, SQL, user.UserToken, user.UserTokenRefresh, user.UserLastLogin, user.UserId)
	helper.PanicIfError(err)
